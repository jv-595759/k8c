...
lxc profile delete k8s-lb >/dev/null 2>&1 && \
...
used_by: []
EOF
lxc profile edit k8s < ./tmp.config >/dev/null 2>&1 && \
rm -f ./tmp.config >/dev/null 2>&1 &
rc_stats
...
then
	...
	lxc profile copy k8s k8s-lb >/dev/null 2>&1 && \
    lxc profile set k8s-lb limits.cpu 1 >/dev/null 2>&1 && \
    lxc profile set k8s-lb limits.memory 1GB >/dev/null 2>&1 && \
    lxc launch ubuntu:20.04 haproxy -p k8s-lb >/dev/null 2>&1 && \
	...
	backend kubernetes-backend
	  ...
	  balance roundrobin\" >> /etc/haproxy/haproxy.cfg" >/dev/null 2>&1 &
	rc_stats
	...
	masterJoinCommand="$(lxc exec kmaster1 -- sh -c 'kubeadm token create --print-join-command 2>/dev/null') --control-plane --certificate-key $(lxc exec kmaster1 -- sh -c 'kubeadm init phase upload-certs --upload-certs --one-output 2>/dev/null | grep -v upload-certs') --ignore-preflight-errors=all"
	joinCommand="$(lxc exec kmaster1 -- sh -c 'kubeadm token create --print-join-command 2>/dev/null') --ignore-preflight-errors=all"
	rc_stats	

	print_op "Pull config file"
	lxc file pull kmaster1/root/.kube/config ~/.kube/config >/dev/null 2>&1 &
	rc_stats
